...
DISCOVEREE=`meshblu-util register -s localhost:3000 -d "{ \"discoverWhitelist\": [\"CHEESE\"] }" -t peter:rocks`
...
# curl http://$AUTH_DEVICE_UUID:$AUTH_DEVICE_TOKEN@localhost:5000/v2/whoami -H "X-AS: $AUTH_DEVICE_UUID"
# curl -vvvv http://$AUTH_DEVICE_UUID:$AUTH_DEVICE_TOKEN@localhost:5000/v2/whoami -H "X-AS: $DISCOVEREE"
