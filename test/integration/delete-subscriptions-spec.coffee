...
describe 'DELETE /v2/devices/:subscriberUuid/subscriptions', ->
  beforeEach (done) ->
    ...
    beforeEach (done) ->
      options =
        auth:
          ...
          password: 'poop-deck'
        json:
          emitterUuid: "another-uuid"
          type: "broadcast"
          foo: 'bar' # should be filtered out

      request.delete "http://localhost:#{@port}/v2/devices/irritable-captian/subscriptions", options, (error, @response, @body) =>
        done error

    it 'should have the correct jobType', ->
      expect(@request.metadata.jobType).to.equal 'RemoveSubscriptions'

    it 'should have the right toUuid', ->
      expect(@request.metadata.toUuid).to.equal 'irritable-captian'

    it 'should have the correct data', ->
      expect(JSON.parse @request.rawData).to.deep.equal {emitterUuid: 'another-uuid', type: 'broadcast'}
